<script lang="ts">
    ...
    let filterStatus = statusFilters["all"];
    ...
    const disputeOrder = (id: number) => async () => {
        const item = items.find((i) => i.order.id == id)!.order;
        const recieve = item.cart
            .map((e) => {
                const coin = r[getOnChainDenom(e[1].info)];
                let amount =
                    (Decimal.fromAtomics(
                        e[1].amount,
                        coin.coinDecimals,
                    ).toFloatApproximation() *
                        item.buyer_risk_share[0]) /
                    item.buyer_risk_share[1];
                return `${amount}${coin.coinDenom}`;
            })
            .join(",\n");
        if (
            await alert(
                `Are you sure you want to dispute the order, you will recieve at least:\n${recieve}`,
            )
        ) {
            await $user!.emporionClient.dispute_order({ order_id: id }, "auto");
            items.find((i) => i.order.id === id)!.order.status = "disputed";
            items = items;
        }
    };
...
<div>
    <Table index="order" rows={items} bind:filter={filterStatus}>
        <svelte:fragment slot="head" let:sort let:sorts>
            ...
            <Select bind:selected={filterStatus}>
        ...
        <svelte:fragment slot="item" let:item>
            {#if !item.isOpen}
                ...
                <div>
                    {item.metaData?.trackingNumber || "No tracking number"}
                </div>
                <div>
            ...
            {#if !!item.isOpen}
                <div class="row-container">
                    ...
                    <div class="expended-content">
                        <div>
                            <h2>Cart</h2>
                            <CartProducts orderId={item.order.id} showRate cart={item.order.cart}></CartProducts>
                        </div>
                        <div class="details">
                            <h2>
                            ...
                            {#if ["fulfilled", "disputed"].includes(item.order.status)}
                                ...
                                <div class="rate">
                                    <span
                                        ><i class="ri-star-line"></i>Review seller</span
                                    >
                                    <div>
                                        <Rate
                            ...
                            {/if}
                            <div class="buttons">
                                {#if ["creating", "pending"].includes(item.order.status)}
                                    <button
                                ...
                                {#if ["accepted"].includes(item.order.status)}
                                    <button
                                        class="dispute button-1"
                                        on:click={disputeOrder(item.order.id)}
                                        >Dispute</button
                                    >
                                    <button
