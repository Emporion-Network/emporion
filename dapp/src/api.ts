...
export class Api  {
    ...
    static notify = (err:string, log:string)=>console[log ? "log" : "error"](log ?? err);
    ...
    async imagesGet(address:string){
        console.log(`${this.baseUrl}/images/${address}`);
    ...
    async metaUpload(meta:ProductMetaData){
        this.post(`${this.baseUrl}/collections//auth/create-product`, meta)
        return true;
    }
    ...
    @catchAndNotify('Could not update delivery address')
    async postalAddressUpdate(postalAddressIndex:number, newPostalAddress:string){
        await this.post(`${this.baseUrl}/auth/update-postal-address`, {
            postalAddressIndex,
            newPostalAddress
        })
    }
